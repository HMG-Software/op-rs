...
pub async fn create_views(configuration: &configuration::Configuration, id: &str, create_views_request: Option<models::CreateViewsRequest>) -> Result<serde_json::Value, Error<CreateViewsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v3/views/{id}", local_var_configuration.base_path, id=crate::apis::urlencode(id));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateViewsError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn list_views(configuration: &configuration::Configuration, filters: Option<&str>) -> Result<(), Error<ListViewsError>> {
    let local_var_configuration = configuration;
...
/// 
pub async fn view_view(configuration: &configuration::Configuration, id: i32) -> Result<(), Error<ViewViewError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v3/views/{id}", local_var_configuration.base_path, id=id);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
